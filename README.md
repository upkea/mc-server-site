...
> 💡 **想要官方通用加群链接（更强）？** 在 QQ 里打开你的群 → 群管理 → 加群设置 → 「加群链接」，
> 复制形如 `https://qm.qq.com/q/xxxx` 的链接，填到 `js/main.js` 顶部 `GROUP.officialUrl` 里。
> 填了之后，「一键加群」按钮在手机和电脑上都会直接跳转官方加群页，不再依赖手机跳转/复制群号。
